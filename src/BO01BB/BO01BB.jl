...
function iterative_procedure(todo, node::Node, pb::BO01Problem, incumbent::IncumbentSet, worst_nadir_pt::Vector{Float64}, round_results, verbose; args...)
    ...
    if pb.param.EPB && length(node.localNadirPts) > 0
        ...
        for i = 1:length(node.localNadirPts)
            ...
            # if pb.param.root_relax nodeChild.RBS.natural_order_vect.sols = copy(node.RBS.natural_order_vect.sols) end 
            # end
        ...
        # todo : copy parent's LBS 
        # if length(node.RBS.natural_order_vect.sols) ≥ 2 
        # if pb.param.root_relax node1.RBS.natural_order_vect.sols = copy(node.RBS.natural_order_vect.sols) end 
        # end
        ...
        # todo : copy parent's LBS 
        # if length(node.RBS.natural_order_vect.sols) ≥ 2 
        # if pb.param.root_relax node2.RBS.natural_order_vect.sols = copy(node.RBS.natural_order_vect.sols) end 
        # end
