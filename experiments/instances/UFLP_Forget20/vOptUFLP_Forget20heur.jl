...
function vOptUFLP(forget::Forget,
    method, fname, outputName, heuristic ; step = 0.01
)
    ...
    elseif method == :bc_rootRelax 
        infos = vSolve( model, method=:bc_rootRelax, verbose=false , heuristic = heuristic)
        println(infos)
    elseif method == :bc_rootRelaxCP 
        infos = vSolve( model, method=:bc_rootRelaxCP, verbose=false, heuristic = heuristic )
        println(infos)
    elseif method == :bb_EPB 
        infos = vSolve( model, method=:bb_EPB, verbose=false , heuristic = heuristic)
        println(infos)
    elseif method == :bc_rootRelaxEPB 
        infos = vSolve( model, method=:bc_rootRelaxEPB, verbose=false , heuristic = heuristic)
        println(infos)
    elseif method == :bc_rootRelaxCPEPB 
        infos = vSolve( model, method=:bc_rootRelaxCPEPB, verbose=false, heuristic = heuristic )
        println(infos)
...
function solve(fname::String, method)
    ...
    vOptUFLP(forget, Symbol(method), forget.name, outputName, heuristic)


    heuristic = true

    folder = "../../results/UFLP_Forget/heuristic"
    if !isdir(folder)
        mkdir(folder)
    end
