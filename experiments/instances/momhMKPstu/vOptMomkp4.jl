...
function vSolveBi01IP(solverSelected, C, A, B, fname, method, strategy, limit)
...
#   end
  ...
  for n in [10, 10, 20, 30, 40, 50] #
    ...
    elseif method == :bc_rootRelax 
      infos = vSolve( Bi01IP, method=:bc_rootRelax, verbose=false, λ_strategy = strategy, λ_limit = limit )
      println(infos)
    elseif method == :bc_rootRelaxCP 
      infos = vSolve( Bi01IP, method=:bc_rootRelaxCP, verbose=false , λ_strategy = strategy, λ_limit = limit)
      println(infos)
    elseif method == :bb_EPB
      infos = vSolve( Bi01IP, method=:bb_EPB, verbose=false )
      println(infos)
    elseif method == :bc_rootRelaxEPB
      infos = vSolve( Bi01IP, method=:bc_rootRelaxEPB, verbose=false , λ_strategy = strategy, λ_limit = limit)
      println(infos)
    elseif method == :bc_rootRelaxCPEPB
      infos = vSolve( Bi01IP, method=:bc_rootRelaxCPEPB, verbose=false , λ_strategy = strategy, λ_limit = limit )
      println(infos)
    elseif method == :bc
    ...
    for strategy = 0:0 # 0:2
        for limit in [0, 2, 3, 4, 6, 8] #[2, 4, 6]
            for method in [
                ...
                vSolveBi01IP(solverSelected, dat.C, dat.A, dat.b, fname, method, strategy, limit) 
            end
        end
    end
