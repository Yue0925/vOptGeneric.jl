...
function vSolveBi01IP(solverSelected, C, A, B, fname, method, heuristic, mixed, mixed2 )
    folder = "../../results/momhMKPstu"
    if !isdir(folder)
        mkdir(folder)
    end
  ...
  result_dir = "../../results/momhMKPstu/" * split(fname, "/")[2] * "/" * split(fname, "/")[3] * "/mixed2"
  if !isdir(result_dir)
    mkdir(result_dir)
  end
  ...
  for n in [10, 10, 20, 30, 40, 50] #
    ...
    elseif method == :bc_rootRelax 
      infos = vSolve( Bi01IP, method=:bc_rootRelax, verbose=false , heuristic = heuristic, mixed = mixed, mixed2 = mixed2)
      println(infos)
...
function main(fname::String)
  ...
  for method in [
    ...
    vSolveBi01IP(solverSelected, dat.C, dat.A, dat.b, fname, method, heuristic, mixed,  mixed2) 
  end
