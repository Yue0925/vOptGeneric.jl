...
function vSolveBi01IP(solverSelected, C, A, B, fname, method, limit)

  # todo : attention folder 
    result_dir = "../../results/momhMKPstu/" * split(fname, "/")[2] * "/" * split(fname, "/")[3]
    if !isdir(result_dir)
      mkdir(result_dir)
    end
    
    folder = result_dir * "/lambda_limit"
    if !isdir(folder)
      mkdir(folder)
    end

    folder = result_dir * "/lambda_limit" * "/" * string(limit)
    if !isdir(folder)
      mkdir(folder)
    end

    folder = result_dir * "/lambda_limit" * "/" * string(limit) * "/" * string(method)
    if !isdir(folder)
      mkdir(folder)
    end
    ...
    for n in [10, 10, 20, 30, 40, 50] 
      ...
      elseif method == :bc_rootRelax 
        infos = vSolve( Bi01IP, method=:bc_rootRelax, verbose=false, LBSexhaustive = exhaustive, λ_limit = limit  )
        println(infos)
      elseif method == :bc_rootRelaxCP 
        infos = vSolve( Bi01IP, method=:bc_rootRelaxCP, verbose=false , LBSexhaustive = exhaustive, λ_limit = limit )
        println(infos)
      elseif method == :bb_EPB
        infos = vSolve( Bi01IP, method=:bb_EPB, verbose=false )
        println(infos)
      elseif method == :bc_rootRelaxEPB
        infos = vSolve( Bi01IP, method=:bc_rootRelaxEPB, verbose=false , LBSexhaustive = exhaustive, λ_limit = limit )
        println(infos)
      elseif method == :bc_rootRelaxCPEPB
        infos = vSolve( Bi01IP, method=:bc_rootRelaxCPEPB, verbose=false , LBSexhaustive = exhaustive, λ_limit = limit )
        println(infos)
      elseif method == :bc
...
function main(fname::String)
    ...
    for limit in [2, 3, 6, 8, 10, 13, 17]
        ...
        # :bb_EPB,
            ...
            vSolveBi01IP(solverSelected, dat.C, dat.A, dat.b, fname, method, limit) 
        end
    end
