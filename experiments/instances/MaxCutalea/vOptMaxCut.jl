...
function vopt_solve(method, outputName; step=0.5) 
    ...
    @addobjective(model, Max, sum([W_bis[i, j]*x[i, j] for i=1:n-1 for j=i+1:n]))
    ...
    (method != :dicho && method != :epsilon) ? writeResults(round(Int64, n*(n-1)/2+n), n*(n-1), inst_name, outputName, method, Y_N, X_E; infos) :
        writeResults(round(Int64, n*(n-1)/2+n), n*(n-1), inst_name, outputName, method, Y_N, X_E; total_time)
