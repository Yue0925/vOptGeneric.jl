...
function desc_grad_stock_2opt(G,sol)

	X = copy(G.X)
	Y = copy(G.Y)
	
	I = copy(sol) #Vector(1:(G.nb_points))
	
	# en pratique le shuffle devrait être fait avant les deux boucles for mais c'est un petit détail
	A = Shuffle.shuffle(Vector(1:(G.nb_points)))
	...
	val=Compute_value_TSP(G, A)
	println("Solution Meta :S=",A)
	println("Valeur: ",val)
	println()
...
# pour faire le dgs2opt rapidement et fournir un pdf
function Find_TSP_dgs2opt(filename)
	
	g = Read_undirected_TSP(filename)
	
	s_ppv= plus_proche_voisin(g)
	
	s = desc_grad_stock_2opt(g,s_ppv)
	
	filename = replace(filename, ".tsp" => "_dgs2opt")
	
	WritePdf_visualization_solution_ordre(g, s, filename)	
