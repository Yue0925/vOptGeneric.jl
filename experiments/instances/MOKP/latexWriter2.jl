...
function comparisonsLambdaLimits(instances::String)
    ...
    avg_time = Dict(m => Dict(n => Dict{Int64, Float64}() for n in avg_n) for m in methods)
    avg_node = Dict(m => Dict(n => Dict{Int64, Float64}() for n in avg_n) for m in methods)
    ...
    for m in methods 
        for n in avg_n
            for λ in λ_limits
                avg_time[m][n][λ] = 0.0 ; avg_node[m][n][λ] = 0.0
            end
        end
    end
    ...
    for m in methods
        for n in keys(avg_count)
            for λ in λ_limits
                avg_node[m][n][λ] = round(avg_node[m][n][λ]/avg_count[n], digits = 2)
                avg_time[m][n][λ] = round(avg_time[m][n][λ]/avg_count[n], digits = 2)
            end
    ...
    for m in methods
        for n in keys(avg_count)
            ...
            ax.set_title("MOKP avg n = $n", fontsize=14)
